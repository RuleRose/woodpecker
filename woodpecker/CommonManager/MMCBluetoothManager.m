...
#define TIMEOUT_DEVICE_SCAN 20.0
#define TIMEOUT_DEVICE_CONNECT 60.0

@interface MMCBluetoothManager ()<CBCentralManagerDelegate>
...
@implementation MMCBluetoothManager
Singleton_Implementation(MMCBluetoothManager);
...
- (void)startScan:(void (^)(NSInteger sendState))callback {
    if ([self checkCentralManagerIsPowerOn]) {
        ...
        NSArray *serviceList = @[ [CBUUID UUIDWithString:SERVICE_UUID_THERMOMETER] ];
...
- (BOOL)checkCentralManagerIsPowerOn {
    if (self.myCentralManager.state == CBCentralManagerStatePoweredOn) {
        ...
        [[XJFHUDManager defaultInstance] showTextHUD:kLocalization(@"alert_core_bluetooth_is_not_power_on")];
        return NO;
    }
}
