...
@interface UNICDatabaseTableManager ()
...
- (void)updateDataBase {
    switch (self.preVersion) {
        case 1: {
            ...
            [[UNICDBOperator defaultInstance] executeSQLs:updateSQL];

            NSString *updateSQL2 = @"alter table UNICCommonTemperatureModel add order_pid TEXT";
            [[UNICDBOperator defaultInstance] executeSQLs:updateSQL2];

            UNICCommonOrderModel *orderCondition = [[UNICCommonOrderModel alloc] init];
            orderCondition.order_type = 2;
            NSArray<UNICCommonOrderModel *> *data = [UNICDBManager searchModelsWithCondition:orderCondition andpage:-1 andOrderby:nil isAscend:NO];
