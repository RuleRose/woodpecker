...
@interface WPMenstrualRegularPopupView()<UIPickerViewDataSource,UIPickerViewDelegate>
@property (nonatomic, strong) UIPickerView *pickerView;
@property (nonatomic, strong) UIView *markView;

@end
@implementation WPMenstrualRegularPopupView
- (instancetype)init
{
    self = [super init];
...
- (void)confirmBtnPressed{
    ...
    if ([_pickerView selectedRowInComponent:0] == 0) {
        if (_regularBlock) {
            _regularBlock(self, NO);
        }
    }else{
        if (_regularBlock) {
            _regularBlock(self, YES);
        }
    }
}
/*
