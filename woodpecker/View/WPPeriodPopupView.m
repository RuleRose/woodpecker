...
@interface WPPeriodPopupView()<UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation WPPeriodPopupView
...
- (instancetype)init
{
    ...
    if ( self )
    {
        ...
        self.titleLabel.text = kLocalization(@"period_distance");
...
- (void)confirmBtnPressed{
    [self hide];
    NSInteger period = [_pickerView selectedRowInComponent:0];
    if (_periodBlock) {
        _periodBlock(self, period);
    }
}
